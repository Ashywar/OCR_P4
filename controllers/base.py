...
class Controller:
    ...
    def add_tournament_random_1(self):
        ...
        new_tournament.add_player(
            Player("Inès-Corinne", "Voisin", "03/19/85", "F", "e5rs9")
        )
        new_tournament.add_player(Player("Léon", "Aubry", "02/04/00", "F", "7ek8He"))
        new_tournament.add_player(
            Player("Michelle ", "Hardy", "09/10/98", "F", "htW109")
        )
        new_tournament.add_player(
            Player("Denise Martineau", "Renault", "02/26/90", "M", "3gJ9C2")
        )
        new_tournament.add_player(
            Player("Auguste", "Regnier", "04/12/95", "M", "45Lcx6")
        )
        new_tournament.add_player(
            Player("Anne-Agathe", "Guerin", "01/01/88", "F", "7856BM")
        )
        new_tournament.add_player(
            Player("Henriette ", "Deschamps", "08/03/01", "F", "5XbBK8")
        )
        new_tournament.add_player(
            Player("Jacqueline Riou", "Lemaitre", "04/23/89", "F", "h69dGf")
        )
    ...
    def add_tournament_random_2(self):
        ...
        new_tournament.add_player(
            Player("Bernadette ", "Guilbert", "08/27/86", "M", "rNj580")
        )
        new_tournament.add_player(
            Player("Xavier", "Bernier", "02/13/99", "M", "foz2v9")
        )
        new_tournament.add_player(
            Player("Auguste ", "Olivier", "02/29/92", "M", "eeP78z")
        )
        new_tournament.add_player(
            Player("Geneviève", "Dupuy", "12/29/89", "F", "t8X6X3")
        )
        new_tournament.add_player(
            Player("Inès-Louise", "Pasquier", "09/06/86", "F", "6H1Ncq")
        )
        new_tournament.add_player(
            Player("Emmanuelle ", "Bourgeois", "06/09/01", "M", "4a1Qkm")
        )
        new_tournament.add_player(
            Player("Claude ", "Meunier", "03/27/96", "M", "0t3lmj")
        )
        new_tournament.add_player(
            Player("Elisabeth ", "Denis", "05/22/85", "F", "89Dr00")
        )
        new_tournament.set_date_begin()
        for player in new_tournament.players:
            player.ranking = random.randint(1, 1000)
