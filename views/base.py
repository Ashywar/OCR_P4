...
class View:
    ...
    def display_menu_tournament_action(self, tournament: Tournament) -> int:
        ...
        header_menu_tournament_action = self.get_all_information_tournament(
            tournament)
        return self.display_menu_generic(
    ...
    def check_input_id(self, input_id) -> boolean:
        if len(input_id) == 6 and input_id[:2].isalpha() and input_id[2:].isdigit():
            return True
        return False
        ...
        for i in range(len(items)):
            if items_check[i] == "check_input_number_of_players":
                ...
                if not self.check_input_validity_birthday(items[i]):
                    return False
            elif items_check[i] == "check_input_id":
                if not self.check_input_id(items[i]):
                    return False
            else:
                return False
        return True
    ...
    def get_string_player_sorted_by_rank_score(self, tournament: Tournament) -> str:
        ...
        if not tournament.finish:
            players_sorted_rank = sorted(
                tournament.players, key=lambda p: p.ranking)
        else:
    ...
    def get_string_player_sorted_by_rank(
        ...
        for player in players_sorted_rank:
            table_body = [
                f"{player.first_name} {player.last_name}", player.ranking]
            table.append(table_body)
        return str(tabulate(table, headers="firstrow", tablefmt="grid"))
    ...
    def get_string_player_sorted_by_name(
        ...
        if tournament_name is None:
            table_header = [
                "Acteurs des tournois ranger par ordre alphabétique:"]
        else:
            table_header = [
    ...
    def display_banner(self):
        ...
        print(
            r"      | | ___  _   _ _ __ _ __   ___  _     __| |/  ___  ___| |__   ___  ___ ___  "
        )
        print(
            r"      | |/ _ \| | | | '__| '_ \ / _ \| |   / _` |  / _ \/ __| '_ \ / _ \/ __/ __| "
        )
        print(
            r"      | | (_) | |_| | |  | | | | (_) | |  | (_| | |  __/ (__| | | |  __/ (__\__ \ "
        )
        print(
            r"      |_|\___/ \__,_|_|  |_| |_|\___/|_|   \__,_|  \___|\___|_| |_|\___|\___|___/ "
            "\n"
        )
